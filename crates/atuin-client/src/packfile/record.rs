...
use crate::{history::HISTORY_TAG, packfile::UploadError, record::sqlite_store::SqliteStore};
...
pub enum PackBodyError {
    ...
    Decrypt(eyre::Report),
    #[error("failed to pack the packfile: {0}")]
    Pack(#[from] PackError),
}
...
impl<'a> PackManifestRecordView<'a> {
    ...
    pub async fn load_encrypted_packed_records(
        &self,
        store: &SqliteStore,
    ) -> Result<impl Iterator<Item = Record<EncryptedData>>, RecordLoadingError> {
    ...
    pub async fn pack_and_upload_records(
        &self,
        store: &SqliteStore,
        key: paseto_v4::Key,
        client: &Client,
    ) -> Result<(), PackAndUploadError> {
        let blob = self.pack_records(store, key)?.await;
        client
            .upload_packfile(self.record.id, self.manifest, blob)
            .await?;
    ...
    fn ia(&self) -> PackIA {
        PackIA {
            ...
            manifest_version: self.record.version,
...
mod tests {
    ...
    // first, and the `pack_records`/`unpack_records` signatures must settle. Behaviors to preserve:
    //   - round_trips: pack then unpack yields the original records (property test over `arb_record`)
