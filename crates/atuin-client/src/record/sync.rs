...
async fn sync_upload(
    ...
    loop {
        ...
        // Ship the page's packfile blobs *before* posting the manifest records, so a manifest
        // that reaches the server always has its packfile. Each blob references history records
        // already uploaded (the `history` tag sorts before `packfile`) plus its own manifest's
        // identity, so the packfiles are independent -- `upload_packed_many` ships them in bounded
        // concurrent batches. A packfile failure fails this page, which retries cleanly next sync
        // (the manifest records here were not posted).
        if tag == PACKFILE_TAG {
            upload_packed_many(&page, store, key, client)
                .await
                .map_err(|e| {
                    error!("failed to upload packfiles: {e:?}");
...
async fn sync_download(
    ...
    loop {
        ...
        // For packfile manifests, ship the packfile's history into the local store BEFORE we
        ...
        if tag == PACKFILE_TAG {
            let expanded = download_packed_many(&page, store, key, client)
                .await
                .map_err(|e| {
                    error!("failed to download packfiles: {e:?}");
                    SyncError::RemoteRequestError { msg: e.to_string() }
                })?;
